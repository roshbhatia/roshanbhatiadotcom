...
export function calculatePositionWithGutterById(id: string, objectWidth: number, viewportWidth: number, gutter?: number) {
  ...
  return rect ? calculatePositionWithGutter(rect, objectWidth, viewportWidth, gutter) : null;
}
...
export function elide(string: string, length = 140, emptyState = '...') {
  if (isEmpty(string)) return emptyState;
  if (string.length < length) return string.trim();
...
export function createSlug(text: any) {
  if (isEmpty(text)) return 'untitled';
...
export function timeAgo(dateInput: Date | string | number): string {
  ...
  if (secondsPast < 0 || isNaN(secondsPast)) return '[INVALID]';
